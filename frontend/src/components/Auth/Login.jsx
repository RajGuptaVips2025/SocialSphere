...
const Login = () => {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center justify-center min-h-screen bg-[#0f172a] space-y-4"
    >
      {/* Login Card */}
      <motion.div
      ...
      {/* ✅ Note Message (Now below the card, centered) */}
      {showNote && (
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5 }}
          className="bg-blue-600/20 border border-blue-500 text-blue-300 text-sm rounded-md p-3 w-[90%] max-w-md text-center relative"
        >
          <button
            onClick={() => setShowNote(false)}
            className="absolute top-2 right-3 text-blue-300 hover:text-blue-100 text-lg font-bold"
          >
            ×
          </button>
